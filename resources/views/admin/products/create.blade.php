...
@section('title', __('admin_product_form.add_title'))
@section('breadcrumb', __('admin_product_form.add_breadcrumb'))
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6">
    ...
    <div class="lg:col-span-2 space-y-4 lg:space-y-5">
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name_en" dir="ltr" value="{{ old('name_en') }}"
                ...
                <div>
                    <label class="admin-label">{{ __('admin_product_form.slug') }} <span class="text-gray-400 font-normal">{{ __('admin_product_form.slug_hint') }}</span></label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="category" class="admin-input" required>
                            <option value="" disabled {{ old('category') ? '' : 'selected' }}>{{ __('admin_product_form.select_category') }}</option>
                    ...
                    <div class="flex items-center justify-between gap-3 mb-2">
                        ...
                        <label class="flex items-center gap-2 cursor-pointer flex-shrink-0">
                            <input type="checkbox" id="offerToggle" onchange="toggleSection('offerPanel', this.checked)"
                                   {{ old('original_price') ? 'checked' : '' }} class="w-4 h-4 accent-[#775a19] cursor-pointer"/>
                            <span class="text-[10px] text-gray-600 tracking-wider uppercase">{{ __('admin_product_form.this_product_on_offer') }}</span>
                        </label>
                    </div>
                    <div id="offerPanel" class="{{ old('original_price') ? '' : 'hidden' }}">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="subtitle_en" dir="ltr" value="{{ old('subtitle_en') }}" class="admin-input"/>
                    </div>
                    <div>
                        <label class="admin-label">{{ __('admin_product_form.subtitle_ar') }}</label>
                        <input type="text" name="subtitle_ar" dir="rtl" value="{{ old('subtitle_ar') }}" class="admin-input"/>
                    </div>
                </div>

                <div>
                    <label class="admin-label">{{ __('admin_product_form.badge_text') }} <span class="text-gray-400 font-normal">{{ __('admin_product_form.badge_text_hint') }}</span></label>
                    <input type="text" name="badge" value="{{ old('badge') }}" class="admin-input"/>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="admin-label">{{ __('admin_product_form.description_en') }}</label>
                        <textarea name="description_en" dir="ltr" rows="4" class="admin-input resize-none" required>{{ old('description_en') }}</textarea>
                    ...
                    <div>
                        ...
                        <textarea name="description_ar" dir="rtl" rows="4" class="admin-input resize-none" required>{{ old('description_ar') }}</textarea>
        ...
        <div class="bg-white border border-gray-200 p-4 sm:p-6">
            ...
            <label class="admin-label">{{ __('admin_product_form.main_image_required') }} <span class="text-gray-400 font-normal">{{ __('admin_product_form.main_image_hint_create') }}</span></label>
            <input type="file" name="image" accept="image/*" required
                   onchange="previewImg(this, 'previewMain')"
                   class="w-full text-[11px] text-gray-600 border border-gray-200 p-2"/>
            <div id="previewMain" class="mt-2 hidden w-full h-40 overflow-hidden bg-gray-100">
                <img class="w-full h-full object-cover"/>
            </div>
            @error('image')<p class="admin-error">{{ $message }}</p>@enderror
        </div>

        @include('admin.products._media_specs', ['product' => null])
    ...
    <div class="space-y-4 lg:space-y-5">
            ...
            <div class="space-y-3">
                ...
                <label class="flex items-center gap-3 cursor-pointer group">
                    <input type="checkbox" name="labels[]" value="{{ $key }}"
                           {{ in_array($key, old('labels', [])) ? 'checked' : '' }}
        ...
        <div class="hidden lg:block space-y-3">
            <button type="submit"
                ...
                {{ __('admin_product_form.create_product') }}
            </button>
